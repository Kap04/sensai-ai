...
export function useCourses() {
  ...
  useEffect(() => {
    ...
    // For MVP, use a default user ID since we're using email as userId
    const userId = typeof user.id === 'string' && user.id.includes('@') ? 1 : user.id;
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/users/${userId}/courses`)
      .then(response => {
        if (!response.ok) {
...
export function useSchools() {
  ...
  useEffect(() => {
    ...
    // For MVP, use a default user ID since we're using email as userId
    const userId = typeof user.id === 'string' && user.id.includes('@') ? 1 : user.id;
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/users/${userId}/orgs`)
      .then(response => {
        if (!response.ok) {
          throw new Error(`Request failed: ${response.status}`);
        }
      ...
      .catch(err => {
        ...
        // For MVP, don't set error to avoid breaking the UI
        setSchools([]);
      })
      .finally(() => {
        setIsLoading(false);
