...
async def create_pdf_documents_table(cursor):
    await cursor.execute(
...
async def create_generated_quizzes_table(cursor):
    await cursor.execute(
    ...
    await cursor.execute(
        """CREATE INDEX idx_generated_quizzes_pdf_id ON generated_quizzes (pdf_document_id)"""
    )


async def create_quiz_questions_table(cursor):
    await cursor.execute(
...
async def create_quiz_submissions_table(cursor):
    await cursor.execute(
...
async def create_quiz_answers_table(cursor):
    await cursor.execute(
...
async def init_db():
    ...
    async with get_new_db_connection() as conn:
        ...
        if exists(sqlite_db_path):
            # Create ALL tables if they don't exist (for existing databases)
            tables_to_create = [
                ...
                ("task_completions", create_task_completion_table),
                ...
                ("pdf_documents", create_pdf_documents_table),
                ("generated_quizzes", create_generated_quizzes_table),
                ("quiz_questions", create_quiz_questions_table),
                ("quiz_submissions", create_quiz_submissions_table),
                ("quiz_answers", create_quiz_answers_table),
            ]
        ...
        try:
            ...
            await create_pdf_documents_table(cursor)
            await create_generated_quizzes_table(cursor)
            await create_quiz_questions_table(cursor)
            await create_quiz_submissions_table(cursor)
            await create_quiz_answers_table(cursor)
